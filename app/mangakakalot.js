...
async search(query, page) {
    ...
    let html;
    try {
        html = await cloudscraper.get(`${this.url}/search/story/${query}?page=${page}`, {
            headers: {
        ...
        console.error('Cloudscraper fetch error:', err.statusCode || err);
        throw new Error(`Failed to fetch search results: ${err.statusCode || err}`);
    }
    ...
    $('.daily-update .panel_story_list .story_item').each((index, element) => {
        ...
        const mangaID = fullUrl.split('/manga/')[1];
        ...
        mangaList.push({ id: mangaID, img: thumbnail, title, author, update, view });
    });
